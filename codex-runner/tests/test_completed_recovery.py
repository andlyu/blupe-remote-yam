...
class CompletedRecoveryTests(unittest.TestCase):
    ...
    def test_settle_failure_replay_reports_once_before_next_observation(self):
        runner, trace = self.setup_recovery()
        runner._provider = Mock()
        failed=trace['trajectory_results'][-1]
        failed.update(status='aborted',code='joint_settle_timeout',message='left wrist missed',step_id=4,details={'recoverable':True})
        runner._recover_finished_trajectory()
        self.assertIsNone(runner._trajectory)
        runner._provider.trajectory_failed.assert_called_once_with('joint_settle_timeout','left wrist missed',2)
        runner._trajectory_result({**failed,'replayed':True})
        runner._provider.trajectory_failed.assert_called_once()

    def test_settle_failure_cannot_resume_with_missing_progress(self):
        runner,trace=self.setup_recovery()
        runner._provider=Mock()
        trace['trajectory_results'][-1].update(status='aborted',code='joint_settle_timeout',step_id=4,details={'recoverable':True})
        trace['trajectory_progress'].pop()
        with self.assertRaisesRegex(RuntimeError,'incomplete or unordered'):
            runner._recover_finished_trajectory()
        runner._provider.trajectory_failed.assert_not_called()
