...
test('expanding and collapsing setup preserves action nodes and reading order', () => {
  ...
  codex.after = (...added) => { group.children=group.children.filter(n=>!added.includes(n)); group.children.splice(group.children.indexOf(codex)+1,0,...added); };
  ...
  nodes.runSettings.open=true; update(); assert.equal(group.parentElement,footer); assert.deepEqual(group.children,[codex,astra,claude,groot,run]); assert.equal(run.textContent,'Run');
  nodes.runSettings.open=false; update(); assert.equal(group.parentElement,prompt); assert.deepEqual(group.children,[run,codex,astra,claude,groot]); assert.equal(run.textContent,'Setup keys and run');
});
