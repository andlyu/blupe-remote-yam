...
function fixture(writeText) {
  ...
  const nodes = {copyCodexPrompt:{dataset:{}},codexCopyStatus:{}};
  let selected;
...
test('copies only the prompt and announces success', async () => {
  ...
  assert.equal(f.nodes.copyCodexPrompt.dataset.copied,'true');
...
test('clipboard denial selects the prompt for manual copying without false success', async () => {
  ...
  assert.notEqual(f.nodes.copyCodexPrompt.dataset.copied,'true');
});
